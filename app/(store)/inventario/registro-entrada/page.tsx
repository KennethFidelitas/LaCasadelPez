...
import { FormularioRegistro } from '@/components/inventario/formulario-registro'
import { Skeleton } from '@/components/ui/display/skeleton'

// ─── Carga de datos ───────────────────────────────────────────────────────────

async function cargarAnimalesActivos() {
...
async function SeccionFormulario() {
  const animales = await cargarAnimalesActivos()
  return <FormularioRegistro animales={animales} />
}
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      ...
      >
        ...
        <ChevronRight className="h-3 w-3" />
        <span className="text-foreground">Registrar entrada</span>
      </nav>

      {/* Encabezado */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Registrar entrada de animales</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Seleccione el animal, indique la cantidad recibida y los datos de la compra.
        </p>
      </div>
